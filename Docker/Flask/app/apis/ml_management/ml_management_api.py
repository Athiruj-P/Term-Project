...
# Author : Athiruj Poositaporn
@ml_management_api.route("/delete_model", methods=['post'])
def delete_model():
    model_type = request.form.get('type')
    model_id = int(request.form.get('model_id'))
    data = Model(id=model_id)
...
# Author : Athiruj Poositaporn
@ml_management_api.route("/get_all_model", methods=['post'])
def get_all_model():
    model_type = request.form.get('type')
    if(model_type == "ml"):
        ml_manager = MLManagement().create_manager()
...
# Author : Athiruj Poositaporn
@ml_management_api.route("/get_model_by_id", methods=['post'])
def get_model_by_id():
    model_type = request.form.get('type')
    model_id = int(request.form.get('model_id'))
    data = Model(id=model_id)
