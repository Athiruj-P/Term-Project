...
export default {
  ...
  // edit_ml_model
  // Description : ฟังก์ชันสำหรับเปลี่ยนข้อมูลของข้อมูลต้นแบบของวัตถุ
  // Author : Athiruj Poositaporn
  async edit_ml_model(data) {
    return await UserAPI.is_authen().then(async (res_authen) => {
      if (res_authen) {
        let from_data = new FormData();
        from_data.append("type", data.type);
        ...
        let recived_data = new Promise((resolve, reject) => {
          axios(config)
            .then((res) => {
              // console.log("Success edit ml model: ", res.data);
              resolve(res);
            })
            .catch((err) => {
              response.status = false;
              // console.log("Error edit ml model: ", err.response.data);
              try {
                response.data = err.response.data;
              } catch (error) {
                response.data = "Server not response.";
  ...
  // delete_ml_model
  // Description : ฟังก์ชันสำหรับเปลี่ยนสถานะข้อมูลต้นแบบของวัตถุ
  // Author : Athiruj Poositaporn
  async delete_ml_model(data) {
    return await UserAPI.is_authen().then(async (res_authen) => {
      if (res_authen) {
        let from_data = new FormData();
        from_data.append("type", data.type);
  ...
  // change_active_ml_model
  // Description : ฟังก์ชันสำหรับเปลี่ยนข้อมูลของข้อมูลต้นแบบของวัตถุตาม id
  // Author : Athiruj Poositaporn
  async change_active_ml_model(data) {
    return await UserAPI.is_authen().then(async (res_authen) => {
      if (res_authen) {
        let from_data = new FormData();
        from_data.append("type", data.type);
  ...
  // get_all_model
  // Description : ฟังก์ชันสำหรับเรียกข้อมูลต้นแบบของวัตถุทั้งหมด
  // Author : Athiruj Poositaporn
