...
class ImageMeasurement:
    def __init__(self):
        ...
        # self.ml_config_path = "C:/Users/First-AP/Desktop/Homework/4th Year/CSI_project/Docker/Flask/app/db_file/ml_model/ml_model_config.cfg"
        self.ref_config_path = os.path.join(parent, db_config.item["db_file_path"], db_config.item["ref_path"],"ref_model_config.cfg")
        # self.ref_config_path = "C:/Users/First-AP/Desktop/Homework/4th Year/CSI_project/Docker/Flask/app/db_file/ref_model/ref_model_config.cfg"
    ...
    # Author : Athiruj Poositaporn
    def measure_2d_obj_size(self,image):
        parent = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    ...
    def measure_3d_obj_size(self,images):
        parent = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        # กำหนดให้ pixelsPerMetric เป็น None ซึ่งใช้ในการคำนวณอัตราส่วนของรูปภาพอ้างอิง
        # เพื่อใช้ค้นหาความยาวด้านของวัตถุเป้าหมาย
        pixelsPerMetric = None
        ...
        try:
            for index in range(len(images)):
                ...
                # ปรับขนาดของรูปภาพใหม่
                input_image = self.resize_with_aspect_ratio(input_image,width=1500)
                ...
                for obj_list in distance_list_of_ml_obj:
                    ...
                    dimA = dA / pixelsPerMetric
                    dimB = dB / pixelsPerMetric
                    object_lable = "[" + self.ml_model_name + "]"
                ...
                retval, buffer = cv2.imencode('.png', origin)
                data = base64.b64encode(buffer)
                data = data.decode('utf-8')

                result_images.append(data)
                result_measurement.append(result_object_data)
                #############################################################
                #############################################################
                
            volume = result_measurement[0]['dimA'] * result_measurement[0]['dimB'] * result_measurement[1]['dimC']
            result = {'img1' : result_images[0] ,'img2' : result_images[1] ,'img1_data' : result_measurement[0],'img2_data' : result_measurement[1] ,'volume':volume,'unit' : self.ref_model_unit,'status' : "success"}
            return result
        except Exception as identifier:
            logger.error("{}.".format(str(identifier)))
            result = {'mes' : str(identifier), 'status' : "system_error"}
