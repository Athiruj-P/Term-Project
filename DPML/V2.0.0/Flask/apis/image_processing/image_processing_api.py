...
def upload_image():
    ...
    try:
        ...
        result_img = image_processor.measure_2d_obj_size(input_image)
...
# Description : Service ของการอัปโหลดรูปภาพและวัดขนาดของวัตถุ
# Author : Athiruj Poositaporn
@image_processing_api.route("/upload_3d_image", methods=['put'])
@jwt_required
def upload_3d_image():
    try:
        ...
        if(not img1 or not img2):
            ...
            file_extension_1 = (img1.filename.split('.')[-1]).lower()
            file_extension_2 = (img2.filename.split('.')[-1]).lower()

        if file_extension_1 not in extension or file_extension_2 not in extension:
            logger.warning("[{}] Wrong file extension.".format(username))
            raise TypeError(err_msg.msg['file_extension'])
        images = []
        
        nparr = np.fromstring(request.files['img1'].read(), np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR) 
        images.append(img)

        nparr = np.fromstring(request.files['img2'].read(), np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR) 
        images.append(img)
        image_processor = image_measurement.ImageMeasurement()
        result_img = image_processor.measure_3d_obj_size(images)
