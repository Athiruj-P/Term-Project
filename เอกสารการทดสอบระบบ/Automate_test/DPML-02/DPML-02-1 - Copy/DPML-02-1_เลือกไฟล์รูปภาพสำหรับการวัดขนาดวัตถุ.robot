...
Choose file "${path}"
    Choose File     css:.file-3d .filepond--browser     ${path}

Dropdown should be enabled
    Element Should Be Enabled   css:#measurement-3d > div.row.d-flex.justify-content-center.align-content-center > div.col-md-3 > select

The url must be "${text}"
    Wait Until Location Is    ${text}     5 
    Click Element   css:#measurement-3d-tab
...
DPML-02-1-1
    ...
    AND The url must be "http://localhost:3000/upload"
    AND Choose file "C:/Users/First-AP/Desktop/Homework/4th Year/CSI_project/Test data/DPML_test_img/all_extension/BMP.bmp"
    AND Wait "1"
    AND Choose file "C:/Users/First-AP/Desktop/Homework/4th Year/CSI_project/Test data/DPML_test_img/all_extension/BMP.bmp"
    AND Wait "1"
