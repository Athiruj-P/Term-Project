...
def max_area(cnts = None):
    ...
    for c in cnts:
        ...
        dB = dist.euclidean((tl[0],tl[1]), (tr[0],tr[1]))
        print('####################')
        print('tl: {}'.format(tl))
        print('tr: {}'.format(tr))
        print('br: {}'.format(br))
...
for img_path in images_path:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            hsv=cv2.cvtColor(cv2.GaussianBlur(crop_img, (7, 7), 0), cv2.COLOR_BGR2HSV)
            ...
            _, binary = cv2.threshold(merge_rgb_edged, 225, 255, cv2.THRESH_BINARY)
            temp_b = ResizeWithAspectRatio(binary,width=1000)
